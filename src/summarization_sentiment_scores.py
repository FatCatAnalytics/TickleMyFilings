...
class SentimentAnalyzer:
    def __init__(self):
        ...
        self.swap_keywords = ['swap', 'derivative', 'hedge']

    def analyze_sentiment(self, text):
        return self.analyzer.polarity_scores(text)

    def contains_swap_keywords(self, text):
        text_lower = text.lower()
        return any(keyword in text_lower for keyword in self.swap_keywords)
...
def main():
    ...
    df['Sentiment'] = df['Summary'].apply(lambda x: sentiment_analyzer.analyze_sentiment(x)['compound'])
    df['Contains_Swap_Keywords'] = df['Summary'].apply(sentiment_analyzer.contains_swap_keywords)

    summary_table = df[['CIK', 'Cluster', 'Cluster_Topics', 'Summary', 'Sentiment', 'Contains_Swap_Keywords']]
    summary_table.to_csv(os.path.join(DATA_DIR, 'outputs/summary_table_sent.csv'), index=False)
